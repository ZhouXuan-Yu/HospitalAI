INSERT INTO patients(patient_id, his_patient_id, display_name, sex, age) VALUES
('P001','HIS-P001','合成患者A','F',66),
('P002','HIS-P002','合成患者B','M',72),
('P003','HIS-P003','合成患者C','F',59),
('P004','HIS-P004','合成患者D','M',68),
('P005','HIS-P005','合成患者E','F',81)
ON CONFLICT (patient_id) DO NOTHING;

INSERT INTO encounters(encounter_id, patient_id, department, diagnosis, scenario, data_version, admitted_at) VALUES
('E001','P001','呼吸内科','社区获得性肺炎','normal',3,'2026-08-03T08:10:00Z'),
('E002-1','P002','呼吸内科','药物过敏确认住院记录','prior_allergy',2,'2026-05-04T08:00:00Z'),
('E002-2','P002','呼吸内科','社区获得性肺炎，第二次入院','confirmed_allergy_second_admission',4,'2026-08-03T08:30:00Z'),
('E003','P003','呼吸内科','社区获得性肺炎','severe_adr',2,'2026-08-03T09:00:00Z'),
('E004','P004','呼吸内科','社区获得性肺炎','cross_department_duplicate_or_conflict',5,'2026-08-03T09:20:00Z'),
('E004-CARD','P004','心内科','心律失常观察','parallel_department',5,'2026-08-03T07:50:00Z'),
('E005','P005','呼吸内科','社区获得性肺炎','critical_lab_missing',1,'2026-08-03T09:45:00Z')
ON CONFLICT (encounter_id) DO NOTHING;

INSERT INTO department_participation(id, encounter_id, department, role, doctor_name, active) VALUES
('DP001','E001','呼吸内科','主管','陈医生',true),
('DP004A','E004','呼吸内科','主管','陈医生',true),
('DP004B','E004-CARD','心内科','参与','林医生',true)
ON CONFLICT (id) DO NOTHING;

INSERT INTO source_identifier_mapping(internal_id, source_system, source_id, object_type, version) VALUES
('P001','HIS','HIS-P001','PatientProfile',1),
('E001','HIS','HIS-E001','Encounter',3),
('P002','HIS','HIS-P002','PatientProfile',1),
('E002-2','HIS','HIS-E002-2','Encounter',4),
('P004','HIS','HIS-P004','PatientProfile',1),
('E004','HIS','HIS-E004','Encounter',5)
ON CONFLICT (internal_id, source_system, object_type) DO NOTHING;

INSERT INTO diagnosis(id, encounter_id, name, status, source_id, collected_at) VALUES
('DX001','E001','社区获得性肺炎','active','HIS-DX001','2026-08-03T08:20:00Z'),
('DX002','E002-2','社区获得性肺炎','active','HIS-DX002','2026-08-03T08:40:00Z'),
('DX003','E003','社区获得性肺炎','active','HIS-DX003','2026-08-03T09:10:00Z'),
('DX004','E004','社区获得性肺炎','active','HIS-DX004','2026-08-03T09:30:00Z'),
('DX005','E005','社区获得性肺炎','active','HIS-DX005','2026-08-03T09:55:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO lab_result(id, encounter_id, code, name, lab_value, unit, missing_status, source_id, collected_at) VALUES
('LAB001','E001','CRP','C反应蛋白','42','mg/L','present','LIS-001','2026-08-03T08:50:00Z'),
('LAB002','E001','CREA','肌酐','76','umol/L','present','LIS-002','2026-08-03T08:50:00Z'),
('LAB003','E002-2','CRP','C反应蛋白','65','mg/L','present','LIS-003','2026-08-03T09:00:00Z'),
('LAB004','E002-2','CREA','肌酐','90','umol/L','present','LIS-004','2026-08-03T09:00:00Z'),
('LAB005','E003','CRP','C反应蛋白','58','mg/L','present','LIS-005','2026-08-03T09:20:00Z'),
('LAB006','E003','CREA','肌酐','84','umol/L','present','LIS-006','2026-08-03T09:20:00Z'),
('LAB007','E004','CRP','C反应蛋白','51','mg/L','present','LIS-007','2026-08-03T09:40:00Z'),
('LAB008','E004','CREA','肌酐','82','umol/L','present','LIS-008','2026-08-03T09:40:00Z'),
('LAB009','E005','CRP','C反应蛋白',NULL,'mg/L','missing','LIS-009','2026-08-03T10:00:00Z'),
('LAB010','E005','CREA','肌酐',NULL,'umol/L','missing','LIS-010','2026-08-03T10:00:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO drug_catalog(drug_code, name, pharmacology_class, status) VALUES
('D-AMOX','阿莫西林克拉维酸钾','青霉素类','active'),
('D-CEF','头孢曲松','头孢菌素类','active'),
('D-AZI','阿奇霉素','大环内酯类','active'),
('D-LEV','左氧氟沙星','喹诺酮类','active'),
('D-VAN','万古霉素','糖肽类','inactive')
ON CONFLICT (drug_code) DO NOTHING;

INSERT INTO allergy_event(id, patient_id, drug_code, drug_name, status, severity, source_id, confirmed_at) VALUES
('ALG-P002-AMOX','P002','D-AMOX','阿莫西林克拉维酸钾','confirmed','high','HIS-ALG-002','2026-05-06T10:00:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO adverse_drug_reaction(id, patient_id, drug_code, drug_name, severity, review_status, source_id, reviewed_at) VALUES
('ADR-P003-LEV','P003','D-LEV','左氧氟沙星','severe','reviewed','PHARM-ADR-003','2026-07-12T11:00:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO medication_order(id, encounter_id, patient_id, drug_code, drug_name, pharmacology_class, department, status, source_id, updated_at) VALUES
('ORD-P004-AZI','E004-CARD','P004','D-AZI','阿奇霉素','大环内酯类','心内科','active','HIS-ORD-004','2026-08-03T08:15:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO medication_exposure(id, patient_id, encounter_id, drug_code, drug_name, started_at, ended_at) VALUES
('EXP-P002-AMOX','P002','E002-1','D-AMOX','阿莫西林克拉维酸钾','2026-05-04T10:00:00Z','2026-05-06T10:00:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO evidence_document(evidence_id, title, status, version, effective_date, scope, locator, text) VALUES
('EV-CAP-001','CAP演示证据集：住院成人初始抗感染路径','demo_unpublished','2026.08-demo','2026-08-03','呼吸内科/社区获得性肺炎','第2页-初始方案','演示证据：候选药物必须来自院内目录；无明确过敏或严重不良反应时，可比较β内酰胺类、头孢菌素类、大环内酯类和喹诺酮类方案。'),
('EV-CAP-002','CAP演示证据集：监测要求','demo_unpublished','2026.08-demo','2026-08-03','呼吸内科/社区获得性肺炎','第4页-监测项目','演示证据：推荐前需检查过敏史、当前有效用药和关键检验；关键检验缺失时不得按正常值处理。'),
('EV-CAP-PUB-001','院内已发布 CAP 用药路径','published','2026.08-hospital','2026-08-03','呼吸内科/社区获得性肺炎','第1页-发布路径','已发布演示证据：CAP 候选方案必须先通过过敏、ADR、重复用药和关键检验规则。')
ON CONFLICT (evidence_id) DO NOTHING;

INSERT INTO document_block(block_id, evidence_id, block_type, page_label, sort_order, text) VALUES
('BLK-CAP-PUB-001','EV-CAP-PUB-001','paragraph','第1页',1,'CAP 候选方案必须先通过过敏、ADR、重复用药和关键检验规则。'),
('BLK-CAP-DEMO-001','EV-CAP-001','paragraph','第2页',1,'演示证据未发布，不得进入正式推荐依据。')
ON CONFLICT (block_id) DO NOTHING;

INSERT INTO evidence_chunk(chunk_id, evidence_id, block_id, chunk_text, keywords, status, created_at) VALUES
('CHK-CAP-PUB-001','EV-CAP-PUB-001','BLK-CAP-PUB-001','CAP 候选方案必须先通过过敏、ADR、重复用药和关键检验规则。','CAP,社区获得性肺炎,过敏,ADR,检验','published','2026-08-03T00:00:00Z'),
('CHK-CAP-DEMO-001','EV-CAP-001','BLK-CAP-DEMO-001','演示证据未发布，不得进入正式推荐依据。','CAP,演示,未发布','demo_unpublished','2026-08-03T00:00:00Z')
ON CONFLICT (chunk_id) DO NOTHING;

INSERT INTO clinical_rule(rule_id, version, name, status, severity, basis, deterministic_handler, published_at) VALUES
('HR-ALG-001','2026.08','已确认药物过敏继承阻断','published-demo','block','项目受控演示规则：确认过敏在后续就诊中继承并阻断对应候选药物。','confirmedAllergyBlock','2026-08-03T00:00:00Z'),
('HR-ADR-001','2026.08','严重不良反应强提醒','published-demo','strong','项目受控演示规则：医院审核的严重 ADR 触发强提醒。','severeAdrAlert','2026-08-03T00:00:00Z'),
('HR-XDEPT-001','2026.08','跨科室当前有效用药复核','published-demo','strong','项目受控演示规则：跨科室当前有效药物或类别相近时提醒复核。','crossDepartmentMedicationAlert','2026-08-03T00:00:00Z'),
('HR-MISS-001','2026.08','关键检验缺失提示','published-demo','info','项目受控演示规则：关键检验缺失不得按正常值处理。','missingCriticalLabAlert','2026-08-03T00:00:00Z')
ON CONFLICT (rule_id, version) DO NOTHING;

INSERT INTO clinical_rule_case(case_id, rule_id, rule_version, title, input_ref, expected_result, status) VALUES
('RC-ALG-001','HR-ALG-001','2026.08','二次入院继承青霉素过敏','E002-2','blocking=true; candidate C-AMOX blocked','active'),
('RC-ADR-001','HR-ADR-001','2026.08','严重 ADR 强提醒','E003','blocking=false; level=strong','active'),
('RC-XDEPT-001','HR-XDEPT-001','2026.08','跨科室阿奇霉素用药提醒','E004','blocking=false; level=strong','active'),
('RC-MISS-001','HR-MISS-001','2026.08','关键检验缺失提示','E005','blocking=false; level=info','active')
ON CONFLICT (case_id) DO NOTHING;

INSERT INTO dose_rule(dose_rule_id, drug_code, indication, patient_group, renal_adjustment_required, regimen_text, status, evidence_id, version, updated_at) VALUES
('DOSE-CAP-CEF-ADULT','D-CEF','社区获得性肺炎','adult',false,'按院内已发布 CAP 路径执行；具体剂量需医生按本院规则表和患者情况确认。','published','EV-CAP-PUB-001','2026.08','2026-08-03T00:00:00Z'),
('DOSE-CAP-AZI-ADULT','D-AZI','社区获得性肺炎','adult',false,'按院内已发布 CAP 路径执行；合并心电风险或跨科室用药时需药师复核。','published','EV-CAP-PUB-001','2026.08','2026-08-03T00:00:00Z')
ON CONFLICT (dose_rule_id) DO NOTHING;
