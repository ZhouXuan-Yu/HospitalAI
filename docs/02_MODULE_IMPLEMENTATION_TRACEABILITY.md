...
| R-012 | 科研数据 | 支撑报告草稿 | 队列、变量、数据质量、冻结、统计、脱敏导出 | 部分完成：研究队列、变量字典、质量检查、冻结版本、统计运行记录、统计产物落盘、脱敏导出记录、导出文件落盘、artifact 回读和报告草稿审核 API 已通；Java Worker 调用 Python 与下载权限待补 | `research` 模块、`research_cohort`、`research_variable`、`research_dataset_quality_check`、`research_analysis_task`、`research_analysis_run`、`research_deidentified_export`、`research_report_draft` | 缺失统计、版本复现、脱敏导出、报告审核 |
...
| R-015 | 异步任务 | 可靠处理解析/Embedding/统计 | PostgreSQL 任务表、租约、重试、死信 | 部分完成：入站事件状态表、处方草稿写入任务、科研统计任务、失败重试和死信状态已建立；租约领取、后台 Worker 调度和解析/Embedding 任务待补 | `async-job`、Java/Python Worker | Worker 崩溃恢复、幂等 |
