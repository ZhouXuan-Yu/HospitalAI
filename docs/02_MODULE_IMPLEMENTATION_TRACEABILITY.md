...
| R-012 | 科研数据 | 支撑报告草稿 | 队列、变量、数据质量、冻结、统计、脱敏导出 | 部分完成：研究队列、变量字典、质量检查、冻结版本、统计运行记录、脱敏导出记录和报告草稿审核 API 已通；Java Worker 调用 Python 与真实产物落盘待补 | `research` 模块、`research_cohort`、`research_variable`、`research_dataset_quality_check`、`research_analysis_run`、`research_deidentified_export`、`research_report_draft` | 缺失统计、版本复现、脱敏导出、报告审核 |
