...
| R-008 | 医生审核 | 保留最终决策权 | 采纳、修改、驳回、差异、理由 | 部分完成：决策落库、推荐状态更新、阻断不可生成草稿；字段级 diff 待补 | `decision` 模块、Web 矩阵 | 修改前后差异、驳回无草稿 |
| R-009 | 药师复核 | 高风险闭环处理 | 审方队列、沟通、处理结果 | 部分完成：强提醒自动创建药师复核任务，支持查询和解决；药师工作台 UI 待补 | `pharmacist-review` 模块 | 阻断、强提醒、跨科室协同 |
| R-010 | HIS 草稿回写 | 不写正式医嘱但可落地 | 幂等、失败、重试、最终回调 | 部分完成：草稿幂等键、状态查询、HIS callback API 已通；真实 adapter 调用与失败重试待补 | `prescription-draft`、HIS adapter | 重复提交、接口失败、状态回调 |
