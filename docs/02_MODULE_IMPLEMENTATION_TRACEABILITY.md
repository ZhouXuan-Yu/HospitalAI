...
|---|---|---|---|---|---|---|
| R-001 | 真实数据接入 | 医生看到可信患者事实 | HIS/EMR/LIS/目录/SSO 适配器、映射版本、同步游标 | 缺口 | `services/core-api/.../integration`、`contracts/openapi/his-adapter.v1.yaml` | 正常快照、缺字段、旧版本、重复事件、接口失败 |
| R-002 | 统一医疗模型 | 同一患者跨就诊不复制 | PatientProfile、Encounter、科室参与、来源映射 | 部分表已建 | `patient-context` 模块、数据库迁移 | 多次入院、转科、会诊、跨科室摘要 |
...
| R-005 | 药品目录 | 候选来自院内可用目录 | 目录版本、状态、适应范围 | 最小雏形 | `drug_catalog`、目录导入器 | 停用药不能作为正常候选 |
...
| R-015 | 异步任务 | 可靠处理解析/Embedding | PostgreSQL 任务表、租约、重试、死信 | 缺口 | `async-job`、Java/Python Worker | Worker 崩溃恢复、幂等 |
| R-016 | 运维交付 | 可上线运行 | Compose、监控、备份、恢复、压测 | 基线初步 | `infra/otel`、`infra/backup` | 容灾恢复、性能压测 |
